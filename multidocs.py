...
def rebuild_faiss_from_doc_store() -> faiss.Index:
    new_index = faiss.IndexFlatIP(VECTOR_DIM)
    if st.session_state.doc_store:
        mat = np.array([row["embedding"] for row in st.session_state.doc_store], dtype=np.float32)
        if len(mat) > 0:
            new_index.add(mat)
...
def ensure_faiss_ready():
    if "faiss_index" not in st.session_state:
        st.session_state.faiss_index = faiss.IndexFlatIP(VECTOR_DIM)
    else:
        cur = st.session_state.faiss_index
        cur_dim = getattr(cur, "d", None)
        if cur_dim != VECTOR_DIM:
            rebuild_faiss_from_doc_store()

# --- early in script ---
if "doc_store" not in st.session_state:
    st.session_state.doc_store = []

# --- after you probe embeddings and know VECTOR_DIM ---
ensure_faiss_ready()
...
def ingest_pdf(pdf_input: t.Union[str, bytes], pdf_name: str, dedup: bool = True, chunk_size: int = 500, batch_size: int = 64):
    ...
    from PyPDF2 import PdfReader
        ...
        # --- Early exit if already ingested ---
    if not dedup:
        already_exists = any(d["pdf_name"] == pdf_name for d in DOC_STORE)
        if already_exists:
            print(f"Skipping ingestion — {pdf_name} already in DOC_STORE")
            return {"doc_id": None, "pages": 0, "chunks": 0, "skipped": True}
...
# Chat with RAG
# ----------------------------
def chat_with_rag(question: str, history: t.Optional[t.List[dict]] = None, k: int = 8, max_tokens: int = 512 ):
    """
    ...
    # Make sure we always use the current session FAISS + doc store
    DOC_STORE = st.session_state.doc_store
    INDEX = st.session_state.faiss_index
